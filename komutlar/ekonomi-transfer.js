  ...
  if(!hesapdurumu) {
    ...
    if(hesapdurumu) {
      if(!hesapismi) {
        ...
        if(hesapdurumu) {
          if(hesapismi) {
            if(bakiye < para) return message.channel.send(`<:hata:637685816197382165>  Para transfer etmek için yeterli paraya sahip değilsiniz.`)
            if(!transhesapdurumu) return message.channel.send(`<:hata:637685816197382165>  Transfer etmek istediğin kullanıcının bir hesabı bulunamadı.`)
            if(transhesapdurumu) {
