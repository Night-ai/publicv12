...
exports.run = async (client, message, args) => {
  ...
  var s = 'tr'
  var m = "<:yee:608977816872288256> `"+client.ayarlar.prefix+"ayarlar destek` yazarak görebilirsiniz."
  var l = "Liste"
  ...
  let prefix = await db.fetch(`prefix_${message.guild.id}`) || `${dil.ayarlar.errors.degismemis} (f?)`;
  
  var ac = client.emojis.get("642475879615823882") //onay tik
  var ka = client.emojis.get("642475975761723432")// çarpı
  var p = client.emojis.get("642475879615823882")
  var tagB_ = client.emojis.get("642475879615823882") //tag
  
if (!a && a !== "destek" && a !== "kapat" && a !== "liste" && a !== "support" && a !== "off" && a !== "list") {
 
  var embed = new Discord.RichEmbed()
.setColor(client.ayarlar.renk)
  .setAuthor(`» ${message.guild.name} | ${dil.ayarlar.header} «`, `https://cdn.discordapp.com/attachments/629782800152920075/631932992243105821/5050_Settings.png`)
  ...
  .addField(dil.ayarlar.kufurengel, db.has(`kufur-engel_${message.guild.id}`) ? `${ac} ${dil.ayarlar.errors.acik}` : `${ka}` ,true)
  ...
  .addField(`Premium Açıkmı?`, db.has(`premod_${message.guild.id}`) ? `${ac} ${dil.ayarlar.errors.acik}` : `${ka}` ,true)
  .addField(dil.ayarlar.sayack, db.has(`sKanal_${message.guild.id}`) ? `${ac} <#${db.fetch(`sKanal_${message.guild.id}`)}>` : `${ka}`, true)
  .addField(`Sayaç Hedefi`, db.has(`sayac_${message.guild.id}`) ? `${ac} ${db.fetch(`sayac_${message.guild.id}`)}` : `${ka}`, true)
  .addField(dil.ayarlar.davetK, db.has(`dKanal_${message.guild.id}`) ? `${ac} <#${db.fetch(`dKanal_${message.guild.id}`)}>` : `${ka}`,true)
  .addField(dil.ayarlar.tag, db.has(`tagB_${message.guild.id}`) ? `<:rf_check:642475879615823882> ` + db.fetch(`tagB_${message.guild.id}`) : `<:rf_xmark:642475975761723432>`, true)
  ...
  var embed = new Discord.RichEmbed()
  .setColor(client.ayarlar.renk)
  .setAuthor(`» ${message.guild.name} ${dil.ayarlar.supportHeader} «`, `https://cdn.discordapp.com/attachments/629782800152920075/631932992243105821/5050_Settings.png`)
...
if (a === "kapat" || a === "off") {
  ...
  if (x === "küfür-engel") {
    db.delete(`kufur-engel_${message.guild.id}`)
