...
exports.run = async (client, msg, args) => {
    ...
    if(reset.includes(args[1])) {
                        if(!db.has(`${msg.author.id}.renk`)) {
                                ...
                                msg.channel.send("<:yasak:619596371674005515> Renk değiştirilmemiş neyi sıfırlayacaksın?")
                                return
                        }
                        db.delete(`${msg.author.id}.renk`)
                    
                        msg.channel.send("<:doru:619596371879657513> Renk başarıyla **sıfırlandı**.")
                        return
                }
                if(!args[1]) {
                        msg.channel.send("<:yasak:619596371674005515> Bir renk kodu veya `sıfırla` yazmalısın!")
 
                        return
                }
                if(args[1].length !== 6) {
                       msg.channel.send("<:yasak:619596371674005515> Renk kodları 6 hane olur. Lütfen düzgün bir renk kodu giriniz. ( Örnek: 25E2D2 )")
